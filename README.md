...
- **Cloud icon** - panel header, always visible, and the only Cloudflare control (no row menu carries one): the filled cloud in the grey of the other header icons = tunnel on (public links), dashed in the same grey = off/unconfigured (private links), the filled cloud in the accent blue, breathing down to nothing and back = switching on or off (still, and filled on one side of a diagonal and an outline on the other, where the system asks for reduced motion); nothing is drawn around the icon in any state; click, Enter or Space toggles, or opens the setup popup when unconfigured; a switch on whose connector does not come up is refused with `cloudflared did not start - see <connector log>` and links stay private
