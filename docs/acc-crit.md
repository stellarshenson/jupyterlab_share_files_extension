...
- [x] `ACC-SHARE-3` **Drag an entry into the file browser copies it** - HIGH; dragging a file or folder entry onto a folder row or the current-directory empty area saves it there - a file as a file, a folder extracted recursively - and the cursor reads as a copy
  ...
  - log: 2026-09-01T19:55:03Z @kj imported from acc-crit-basic-sharing.md; verified live against the test.user peer on a self-signed hub
  - log: 2026-09-01T19:56:33Z @kj edited test-tags (replaced)
- [x] `ACC-SHARE-4` **Right-click offers Download and Save** - MEDIUM; right-clicking an entry offers Download (browser download, no credentials) and Download to Current Folder (server-side save into the file browser's current directory)
  - test-tags: MANUAL
  - log: 2026-09-01T19:55:03Z @kj imported from acc-crit-basic-sharing.md; verified live against the test.user peer on a self-signed hub
  - log: 2026-09-01T19:56:33Z @kj edited test-tags (replaced)
  - log: 2026-09-24T19:43:00Z @kj amended text "right-clicking an entry offers Download (browser download, no credentials) and Save to Current Folder (server-side save into the file browser's current directory)" -> "right-clicking an entry offers Download (browser download, no credentials) and Download to Current Folder (server-side save into the file browser's current directory)"
...
- [x] `ACC-SHARE-182` **The command line can connect to a host whose certificate the system does not trust** - MEDIUM; Without a flag, `jupyterlab_share_files connect <link>` fails when the link's host uses a certificate the system does not trust, such as a self-signed one, says why and shows the certificate's fingerprint. With `--trust-certificate`, the command connects anyway: it prints the host and the certificate's fingerprint, and the connection keeps that certificate until it is removed, the same as choosing Trust in the panel's dialog (ACC-SHARE-181). A certificate that cannot be used even when trusted, such as an expired one, is still refused.
  - evidence: 2026-09-23 test_the_command_line_trusts_a_certificate_only_when_told (the CLI against a real lab server and a loopback https peer: without the flag it stops and names --trust-certificate, with it the connection stores the peer's certificate and the output carries host and fingerprint, an expired certificate is still refused; fails on the CLI without the flag and with the flag ignored); test_connect_lists_a_share_s_entries_read_through_the_lab; pytest 480 passed 8 skipped
  - test: pytest: the CLI's connect against a loopback https peer with a self-signed certificate fails without the flag, connects with it, and the stored connection carries the certificate; with the flag, an expired certificate is still refused
  - test-tags: UNIT
  - log: 2026-09-23T21:35:07Z @kj added
  - log: 2026-09-23T21:39:57Z @kj closed
  - log: 2026-09-23T21:53:25Z @kj amended text "Without a flag, `jupyterlab_share_files connect <link>` fails when the link's host uses a certificate the system does not trust, such as a self-signed one, and says why. With `--trust-certificate`, the command connects anyway: it prints the host and the certificate's fingerprint, and the connection keeps that certificate until it is removed, the same as choosing Trust in the panel's dialog (ACC-SHARE-181). A certificate that cannot be used even when trusted, such as an expired one, is still refused." -> "Without a flag, `jupyterlab_share_files connect <link>` fails when the link's host uses a certificate the system does not trust, such as a self-signed one, says why and shows the certificate's fingerprint. With `--trust-certificate`, the command connects anyway: it prints the host and the certificate's fingerprint, and the connection keeps that certificate until it is removed, the same as choosing Trust in the panel's dialog (ACC-SHARE-181). A certificate that cannot be used even when trusted, such as an expired one, is still refused."
  - log: 2026-09-23T21:53:25Z @kj 2026-09-23 the refusal shows the SHA-256 fingerprint; every CLI call by connection key encodes the key as one path segment (a standalone key holds https://), so disconnect, pick-up, send-to-request and the entry listing reach the connection; the real-lab test now also disconnects by key; test_a_standalone_connection_key_is_one_path_segment
- [x] `ACC-SHARE-183` **The Connect button is the right end of the link input and cannot be pressed while the input is empty or holds only spaces** - LOW; The Connect button sits against the link input with no gap, as the input's right end, at the input's height. While the input is empty or holds only spaces, the button is a faded outline and is disabled, so a click or Enter does nothing. With a link in the input it is filled in the colour of the theme's default icon. While a connection is being made the button is disabled, keeps its fill faded so the row does not look empty, and carries aria-busy. The faded states are drawn with opacity, so they stay faded in a high contrast theme. Pressing Connect moves the keyboard focus to the input, so disabling the button does not drop the focus to the page, and a dialog the connection opens returns the focus there. A connection that ends without a row keeps the link and the button filled; a connection that adds a row empties the input and the button is a faded outline again. The same row shows in hub mode and standalone.
  - evidence: 2026-09-24 galata mock hub 'the Connect button is the right end of the input and is disabled while the input is empty or holds only spaces' passes, and 9 panel mutants (no connecting flag, no overlap margin, no initial state, disabled keeps its fill, input listener ignoring the flag, no opacity, no busy fill rule, no aria-busy, no input focus) each fail it; mock hub 53 passed, standalone 35, jest 45, lint 0 warnings; light, dark and Dark High Contrast renders show a faded outline, a fill and a faded fill; five-lens review rounds 3 and 4 clean
  - test: mock hub: the button's left edge lies on the input's right border at the same top and height; empty and spaces leave it disabled with no fill and below full opacity; a link fills it at full opacity; while the password dialog is open it is disabled, aria-busy true, filled and below full opacity, even after an input event; Cancel returns the focus to the input, keeps the link, re-enables the button and sets aria-busy false; a completed connection empties the input and leaves an unfilled disabled button
  - test-tags: FUNCTIONAL
  - mechanism: 2026-09-24T13:02:34Z @kj src/widget.ts syncConnect sets the button's disabled state from the trimmed input value and a connecting flag, and aria-busy from the flag, on every input event and at the end of connectToLink; the click handler focuses the input before it disables the button, so the focus does not drop to the page and a dialog returns it to the input; style/base.css draws :disabled as a transparent outline in the fill colour at opacity 0.5, [aria-busy='true'] restores the fill under that opacity, and a -1px left margin lays the button's left border over the input's right border
  - log: 2026-09-24T12:14:10Z @kj added
  - log: 2026-09-24T12:38:25Z @kj amended title "The Connect button is the right end of the link input and can be pressed only with a link in it" -> "The Connect button is the right end of the link input and cannot be pressed while the input is empty or holds only spaces"; text "The Connect button sits against the link input with no gap, as the input's right end, at the input's height. While the input is empty or holds only spaces, and while a connection is being made, the button is a dim outline and is disabled, so a click or Enter does nothing. With a link in the input it is filled in the colour of the theme's default icon. A connection that ends without a row keeps the link and the button filled; a connection that adds a row empties the input and the button is dim again. The same row shows in hub mode and standalone." -> "The Connect button sits against the link input with no gap, as the input's right end, at the input's height. While the input is empty or holds only spaces, the button is a faded outline and is disabled, so a click or Enter does nothing. With a link in the input it is filled in the colour of the theme's default icon. While a connection is being made the button is disabled, keeps its fill faded so the row does not look empty, and reports itself busy to screen readers. The faded states are drawn with opacity, so they stay faded in a high contrast theme. A connection that ends without a row keeps the link and the button filled; a connection that adds a row empties the input and the button is a faded outline again. The same row shows in hub mode and standalone."
  - log: 2026-09-24T12:38:40Z @kj edited test (replaced)
  - log: 2026-09-24T13:02:34Z @kj amended text "The Connect button sits against the link input with no gap, as the input's right end, at the input's height. While the input is empty or holds only spaces, the button is a faded outline and is disabled, so a click or Enter does nothing. With a link in the input it is filled in the colour of the theme's default icon. While a connection is being made the button is disabled, keeps its fill faded so the row does not look empty, and reports itself busy to screen readers. The faded states are drawn with opacity, so they stay faded in a high contrast theme. A connection that ends without a row keeps the link and the button filled; a connection that adds a row empties the input and the button is a faded outline again. The same row shows in hub mode and standalone." -> "The Connect button sits against the link input with no gap, as the input's right end, at the input's height. While the input is empty or holds only spaces, the button is a faded outline and is disabled, so a click or Enter does nothing. With a link in the input it is filled in the colour of the theme's default icon. While a connection is being made the button is disabled, keeps its fill faded so the row does not look empty, and carries aria-busy. The faded states are drawn with opacity, so they stay faded in a high contrast theme. Pressing Connect moves the keyboard focus to the input, so disabling the button does not drop the focus to the page, and a dialog the connection opens returns the focus there. A connection that ends without a row keeps the link and the button filled; a connection that adds a row empties the input and the button is a faded outline again. The same row shows in hub mode and standalone."
  - log: 2026-09-24T13:02:34Z @kj edited test (replaced)
  - log: 2026-09-24T13:15:55Z @kj closed
- [x] `ACC-SHARE-184` **Every entry of the panel's menus carries an icon** - LOW; Every entry of every context menu the panel opens - share, request and connection rows, their files and uploads, and the New menu - shows an icon beside its label, drawn at the size JupyterLab's own menus use and in the theme's icon colour: a link for Copy Link, a box with an arrow for Open in Browser, a download arrow for Download and for Download to Current Folder, a zipped folder for Save as Zip, a lock for the password, a bin for Delete, a pencil for Rename, a cross for Remove from Share, a folder for Show in File Browser, copy and paste as in the file browser, the broken link for Disconnect, the refresh arrows for Connect Again.
  - evidence: 2026-09-24 galata mock hub 'every entry of the share, request and New menus carries an icon' and the menuOf helper's icon check over the upload, hub file and connected menus, standalone 'every entry of a share row and a share entry menu carries an icon'; mutants dropping the Save as Zip icon and the Copy icon each fail; mock hub 54 passed, standalone 36, jest 45, lint 0 warnings; dark and light screenshots show 16px icons
  - test: mock hub: the share, request and New menus, a hub share's file menu, an upload's menu, a connected share's row and file menus each show an svg in every command entry's icon slot; standalone: a share row's and a share entry's menus the same
  - test-tags: FUNCTIONAL
  - mechanism: 2026-09-24T13:40:57Z @kj every share-files-panel command in src/widget.ts _registerCommands carries an icon (LabIcons from @jupyterlab/ui-components and src/icons.ts, a new zipIcon); the panel builds its menus with MenuSvg, which applies JupyterLab's menuItem icon style so every icon draws at 16px
  - log: 2026-09-24T13:40:57Z @kj added
  - log: 2026-09-24T13:47:13Z @kj closed
  - log: 2026-09-24T19:43:00Z @kj amended text "Every entry of every context menu the panel opens - share, request and connection rows, their files and uploads, and the New menu - shows an icon beside its label, drawn at the size JupyterLab's own menus use and in the theme's icon colour: a link for Copy Link, a box with an arrow for Open in Browser, a disk for Save to Current Folder, a zipped folder for Save as Zip, a lock for the password, a bin for Delete, a pencil for Rename, a cross for Remove from Share, a folder for Show in File Browser, copy and paste as in the file browser, the broken link for Disconnect, the refresh arrows for Connect Again, a download arrow for Download." -> "Every entry of every context menu the panel opens - share, request and connection rows, their files and uploads, and the New menu - shows an icon beside its label, drawn at the size JupyterLab's own menus use and in the theme's icon colour: a link for Copy Link, a box with an arrow for Open in Browser, a download arrow for Download and for Download to Current Folder, a zipped folder for Save as Zip, a lock for the password, a bin for Delete, a pencil for Rename, a cross for Remove from Share, a folder for Show in File Browser, copy and paste as in the file browser, the broken link for Disconnect, the refresh arrows for Connect Again."; reason: the save entries became Download to Current Folder with the download arrow, and Set Password took an own lock (ACC-SAVE-187, ACC-SHARE-188)
- [x] `ACC-SHARE-188` **The Set Password icon matches the other menu icons** - LOW; the lock icon of Set Password is painted in the colour of the other menu icons (--jp-inverse-layout-color3); JupyterLab's lockIcon carries jp-icon4 and read darker
  - evidence: galata save-record.spec.ts asserts the Set Password icon fill equals the Copy Link icon fill; a jp-icon4 mutant fails with rgb(117, 117, 117) against rgb(97, 97, 97); green 2026-09-24: mock hub 55, standalone 38, pytest 494, jest 45
  - mechanism: 2026-09-24T19:08:01Z @kj an own passwordIcon in src/icons.ts: the Material lock path under class jp-icon3, so JupyterLab paints it like the other menu icons
  - test: open a share menu; assert the lock icon's computed fill equals the fill of the Copy Link icon
  - test-tags: FUNCTIONAL
...
- [x] `ACC-CLOUD-160` **The cloud icon wears the header's own colours** - LOW; the cloud icon, off and on, is painted in the colour the other header icons are painted in (--jp-inverse-layout-color3, the .jp-icon3 fill); only a switch in flight takes the accent; it is not green in any state
  - evidence: galata keyboard.spec.ts 'the cloud icon draws its state colour' compares the cloud's fill off and on with the New button's .jp-icon3 fill, not the accent or success colour; hub-mode.spec.ts the same; a --jp-ui-font-color2 mutant fails both; green 2026-09-24: mock hub 55, standalone 38, pytest 494, jest 45
  - test: open the panel on both themes and compare the cloud icon's computed fill, off and on, with the fill of the New button's .jp-icon3 icon
  - test-tags: FUNCTIONAL
  - mechanism: 2026-09-24T19:43:00Z @kj the indicator's base colour is --jp-inverse-layout-color3, the fill JupyterLab's .jp-icon3 rule gives the other header icons; only jp-mod-connecting sets --jp-brand-color1
  - log: 2026-09-20T13:14:00Z @kj added
  - log: 2026-09-20T13:24:15Z @kj edited text
  - log: 2026-09-20T14:01:51Z @kj closed
  - log: 2026-09-24T17:11:31Z @kj amended text "the cloud icon takes the same colour as the other header icons when off and the same accent as an active header control when on; it is not green in either state" -> "the cloud icon takes the same grey as the other header icons when off and when on; only a switch in flight takes the accent; it is not green in any state"; reason: owner 2026-09-24: on is grey, only the switch in flight is blue
  - log: 2026-09-24T17:11:39Z @kj owner 2026-09-24: on is the filled cloud in the grey of the other header icons, not the accent. Gated in keyboard.spec.ts: the on colour equals the New button's computed colour and differs from --jp-brand-color1 and --jp-success-color1; hub-mode.spec.ts compares the established icon with the New button
  - log: 2026-09-24T18:54:02Z @kj amended text "the cloud icon takes the same grey as the other header icons when off and when on; only a switch in flight takes the accent; it is not green in any state" -> "the cloud icon, off and on, is painted in the colour the other header icons are painted in (--jp-inverse-layout-color3, the .jp-icon3 fill); only a switch in flight takes the accent; it is not green in any state"; reason: DEF-PANEL-127: the text colour it took is darker than every header icon
  - log: 2026-09-24T19:43:00Z @kj edited test "open the panel on both themes and compare the cloud icon's computed colour with the neighbouring header buttons in both states" -> "open the panel on both themes and compare the cloud icon's computed fill, off and on, with the fill of the New button's .jp-icon3 icon"
  - log: 2026-09-24T19:43:00Z @kj mechanism updated "2026-09-20T13:14:00Z @kj the indicator's colour comes from the same variables the header buttons use instead of a success colour" -> "the indicator's base colour is --jp-inverse-layout-color3, the fill JupyterLab's .jp-icon3 rule gives the other header icons; only jp-mod-connecting sets --jp-brand-color1"
  - log: 2026-09-24T19:50:18Z @kj edited test-tags "MANUAL" -> "FUNCTIONAL"; evidence "galata keyboard.spec.ts 'the cloud icon switches from the keyboard' resolves both variables in the page and asserts the active colour equals --jp-brand-color1 and differs from --jp-success-color1; standalone suite 31 passed" -> "galata keyboard.spec.ts 'the cloud icon draws its state colour' compares the cloud's fill off and on with the New button's .jp-icon3 fill, not the accent or success colour; hub-mode.spec.ts the same; a --jp-ui-font-color2 mutant fails both; green 2026-09-24: mock hub 55, standalone 38, pytest 494, jest 45"
- [x] `ACC-CLOUD-162` **Switching is told from on by colour and the breath** - HIGH; the header cloud icon's on and switching states draw the same filled cloud: on is still, in the colour of the other header icons; switching is the accent, breathing down to nothing and back, for at least one whole breath however fast the switch lands; the panel has no reduced-motion look; nothing is drawn around the icon in any state
  - evidence: galata server and hub 'answers at once' tests: lowest opacity under 0.05, at least 2400 ms; hub pending test: accent on svg, no box-shadow, breath under reduced motion; a no-hold mutant fails both; green 2026-09-24: mock hub 55, standalone 38, pytest 494, jest 45
  - related: ACC-CLOUD-160 - the owner's colour instruction this keeps, without leaving hue as the only channel
  - test: keyboard.spec.ts 'a switch the server answers at once still shows one whole breath' and hub-mode.spec.ts 'a switch the hub answers at once still shows one whole breath' record the lowest opacity each frame while switching; the colour tests compare on and switching; style/base.css has no prefers-reduced-motion rule
  - test-tags: FUNCTIONAL
  - mechanism: 2026-09-24T19:43:01Z @kj on is the filled cloud, still, in --jp-inverse-layout-color3; switching is the same cloud in --jp-brand-color1 under share-files-breathe (opacity 1 to 0, 2.4s cubic-bezier(0.8, 0, 0.2, 1)), held by _toggleTunnel for CLOUD_BREATH_MS whatever the answer time
  ...
  - log: 2026-09-24T17:11:31Z @kj amended title "Switching is told from on by the glyph" -> "Switching is told from on by colour and the breath"; text "the header cloud icon's "on" and "switching" states carry the same accent, so the glyph separates them - the filled cloud is on, the dashed silhouette is a switch in flight - and the switching state keeps full strength where the blink is suppressed (`prefers-reduced-motion`) instead of dimming to 1.98:1, under the 3:1 bar a non-text control has to clear" -> "the header cloud icon's on and switching states draw the same filled cloud: on is still, in the grey of the other header icons; switching is the accent, breathing down to nothing and back; where the system asks for reduced motion the switching cloud holds still in the accent; nothing is drawn around the icon in any state"; reason: owner 2026-09-24: switching is the solid accent cloud breathing, sharp and to zero, and no rim of any kind
  - log: 2026-09-24T17:11:46Z @kj owner 2026-09-24: breath is opacity 1 to 0, 2.4s cubic-bezier(0.8, 0, 0.2, 1); reduced motion holds it still; no ring or focus mark. Gated in hub-mode.spec.ts: waiting draws the on svg in the accent, opacity under 0.05, box-shadow none; keyboard.spec.ts: connecting and on colours differ, focus draws nothing
  - log: 2026-09-24T17:14:09Z @kj amended text "the header cloud icon's on and switching states draw the same filled cloud: on is still, in the grey of the other header icons; switching is the accent, breathing down to nothing and back; where the system asks for reduced motion the switching cloud holds still in the accent; nothing is drawn around the icon in any state" -> "the header cloud icon's on and switching states draw a filled cloud: on is still, in the grey of the other header icons; switching is the accent, breathing down to nothing and back; where the system asks for reduced motion the switching cloud holds still in the accent, filled on one side of a diagonal and an outline on the other; nothing is drawn around the icon in any state"; reason: owner 2026-09-24: the reduced-motion switching look is a cloud split along a diagonal, filled on one side and an outline on the other
  - log: 2026-09-24T17:14:20Z @kj owner 2026-09-24: under reduced motion the switching svg shows its outline path and clips its fill path to polygon(0 0, 0 100%, 100% 100%). Gated in hub-mode.spec.ts: normally the fill is unclipped and the outline hidden; under emulated reduced motion the fill clip is a polygon and the outline shows
  - log: 2026-09-24T18:54:02Z @kj amended text "the header cloud icon's on and switching states draw a filled cloud: on is still, in the grey of the other header icons; switching is the accent, breathing down to nothing and back; where the system asks for reduced motion the switching cloud holds still in the accent, filled on one side of a diagonal and an outline on the other; nothing is drawn around the icon in any state" -> "the header cloud icon's on and switching states draw the same filled cloud: on is still, in the colour of the other header icons; switching is the accent, breathing down to nothing and back, for at least one whole breath however fast the switch lands; the panel has no reduced-motion look; nothing is drawn around the icon in any state"; reason: owner 2026-09-24: reduced motion dropped entirely; DEF-PANEL-126: a fast switch showed no breath
  - log: 2026-09-24T19:43:01Z @kj edited test "the four-state walk in `ui-tests/tests/keyboard.spec.ts` reads the icon's svg at connecting and at on and they differ; the reduced-motion block in `style/base.css` sets `animation: none` and no opacity" -> "keyboard.spec.ts 'a switch the server answers at once still shows one whole breath' and hub-mode.spec.ts 'a switch the hub answers at once still shows one whole breath' record the lowest opacity each frame while switching; the colour tests compare on and switching; style/base.css has no prefers-reduced-motion rule"
  - log: 2026-09-24T19:43:01Z @kj mechanism updated "2026-09-20T16:03:37Z @kj "on" takes `--jp-brand-color1` by the owner's instruction, and "switching" takes the same accent, so hue cannot carry the difference; the dashed silhouette is the second channel and it survives both a suppressed blink and a colour-blind reading" -> "on is the filled cloud, still, in --jp-inverse-layout-color3; switching is the same cloud in --jp-brand-color1 under share-files-breathe (opacity 1 to 0, 2.4s cubic-bezier(0.8, 0, 0.2, 1)), held by _toggleTunnel for CLOUD_BREATH_MS whatever the answer time"
  - log: 2026-09-24T19:50:43Z @kj edited evidence "the four-state walk in `ui-tests/tests/keyboard.spec.ts` reads the icon's svg at connecting and at on and asserts they differ - 17 keyboard tests passed; `style/base.css` reduced-motion block now sets `animation: none` alone, so the switching glyph holds the accent's 3.97:1 light and 3.22:1 dark instead of dimming to 1.98:1; full runs after the change: galata standalone 31 passed, hub 26 passed, pytest 407 passed 8 skipped, jest 44 passed, `jlpm run lint:check` exit 0" -> "galata server and hub 'answers at once' tests: lowest opacity under 0.05, at least 2400 ms; hub pending test: accent on svg, no box-shadow, breath under reduced motion; a no-hold mutant fails both; green 2026-09-24: mock hub 55, standalone 38, pytest 494, jest 45"
...
- [x] `ACC-HUBM-128` **Upload fetch action** - HIGH; each upload row under a request offers Download to Current Folder in its context menu, which posts the fetch route with the file browser's current directory and reports the landed path; the row carries no button for it
  ...
  - test: mock hub lists one upload; the row has no button and its menu reads Download to Current Folder with an icon; choose it and assert the mock received dest and the lab token
  - test-tags: E2E
  - log: 2026-09-03T20:51:19Z @kj added
  - log: 2026-09-03T21:32:43Z @kj closed: \_fetchUploadFlow posts api/requests/<id>/uploads/<uid>/fetch with the current dir
  - log: 2026-09-24T13:40:50Z @kj amended text "each upload row under a request offers Fetch to current folder which posts the fetch route with the file browser's current directory and reports the landed path" -> "each upload row under a request offers Save to Current Folder in its context menu, which posts the fetch route with the file browser's current directory and reports the landed path; the row carries no button for it"
  - log: 2026-09-24T13:40:50Z @kj edited test (replaced)
  - log: 2026-09-24T19:43:00Z @kj amended text "each upload row under a request offers Save to Current Folder in its context menu, which posts the fetch route with the file browser's current directory and reports the landed path; the row carries no button for it" -> "each upload row under a request offers Download to Current Folder in its context menu, which posts the fetch route with the file browser's current directory and reports the landed path; the row carries no button for it"
  - log: 2026-09-24T19:43:00Z @kj edited test "mock hub lists one upload; the row has no button and its menu reads Save to Current Folder with an icon; choose it and assert the mock received dest and the lab token" -> "mock hub lists one upload; the row has no button and its menu reads Download to Current Folder with an icon; choose it and assert the mock received dest and the lab token"
...
- [x] `ACC-HUBM-176` **A protected record asks for its password once and retries** - MEDIUM; Connecting to a record the owner gave a password answers with the reason rather than a failure, the panel asks for the password in one dialog whose confirm reads Connect, and the retry carries it. The lab unlocks through the link's unlock form and keeps the fs_unlock cookie in memory only, sending it with every later request for that record; the password is kept in the lab's 0600 config, so a restart unlocks again. A wrong password says so and leaves the input open, cancelling adds no row, and too many tries says to wait.
  ...
  - log: 2026-09-24T09:20:54Z @kj 2026-09-24 the 'password changed' label wraps inside its row: the row header takes min-height 24px and the label line-height 1.2; hub-mode.spec.ts 'a password the owner changes after the connect...' checks the label lies inside its row with a longer record name (fails on the previous CSS: the label started 12 px above the row)
- [x] `ACC-HUBM-177` **A record the user already owns is refused by name** - MEDIUM; Connecting to a record this user owns says so - the row is already in the panel - instead of adding a second row for it. The lab compares the link's record id with the ids of the user's own records that the hub lists.
  ...
  - log: 2026-09-23T14:06:46Z @kj edited text and test (replaced) and test-tags (replaced)
  - log: 2026-09-23T14:32:39Z @kj closed
- [x] `ACC-HUBM-178` **A connected share saves its files and folders, or the whole share unpacked or zipped, into the current folder** - HIGH; A connected share sits in the Connected section, expands to its files and folders, and offers Download to Current Folder on each file and each folder, and on the row Download to Current Folder and Save as Zip. The lab downloads the bytes through the link into the file browser's current folder: one file from /d/<path>, a folder file by file, the whole share from /archive, kept as the zip or unpacked. A save lands in a staging folder only it names and moves to the free name when complete, so a save that fails part way leaves nothing behind. The row carries no rename, no remove, no delete, no password control and no Cloudflare control, because none of those are the connecting user's to make.
  ...
  - log: 2026-09-23T14:32:39Z @kj closed
  - log: 2026-09-24T19:43:00Z @kj amended text "A connected share sits in the Connected section, expands to its files and folders, and offers Save to Current Folder on each file and each folder, and on the row Save Record to Current Folder and Save Record as Zip. The lab downloads the bytes through the link into the file browser's current folder: one file from /d/<path>, a folder file by file, the whole share from /archive, kept as the zip or unpacked. A save lands in a staging folder only it names and moves to the free name when complete, so a save that fails part way leaves nothing behind. The row carries no rename, no remove, no delete, no password control and no Cloudflare control, because none of those are the connecting user's to make." -> "A connected share sits in the Connected section, expands to its files and folders, and offers Download to Current Folder on each file and each folder, and on the row Download to Current Folder and Save as Zip. The lab downloads the bytes through the link into the file browser's current folder: one file from /d/<path>, a folder file by file, the whole share from /archive, kept as the zip or unpacked. A save lands in a staging folder only it names and moves to the free name when complete, so a save that fails part way leaves nothing behind. The row carries no rename, no remove, no delete, no password control and no Cloudflare control, because none of those are the connecting user's to make."; reason: the save entries read Download to Current Folder and Save as Zip since 1.2.55 and 2026-09-24
...
- [x] `ACC-HUBM-180` **A connected request takes dropped files and folders and tints its row while they upload** - HIGH; Files and folders dragged from the file browser onto a connected request's row, or pasted onto it, upload into that request. The lab sends each file to the link's /u route as the request page does, one file per request under its own name; a folder's files go up one by one and arrive without the folder structure, because the request takes a single file name, and the excluded-names catalogue applies at every depth. While the upload runs the row wears the progress layer every other transfer wears (ACC-PROG-172) at the fraction the lab has sent, and no other row does; when it ends a notification names how many files went up, and a refusal names its reason in words: over the request's limit, a name the hub refuses, or the hub low on storage. A drop or paste onto a row that says closed, password changed or untrusted sends nothing, a notification gives the row's reason, and a cut stays on the clipboard for the next try.
  ...
  - log: 2026-09-24T09:20:54Z @kj 2026-09-24 a file over the limit the request page states (var cap, kept from the last read) is refused as over_cap before its body is sent (test_a_file_over_the_request_s_limit_is_refused_before_it_is_sent); an upload into a request its owner closes meanwhile says 'The upload to <name> stopped: <reason>' once (hub-mode.spec.ts 'an upload into a request its owner closes meanwhile says it stopped'); both fail on the previous code
  - log: 2026-09-24T09:39:20Z @kj 2026-09-24 the limit check moved into the upload answer: a file over the page's limit is named with the limit ('big.txt is larger than the 2 B this request accepts - nothing was sent', 413) and no file of the drop is sent (test_a_file_over_the_request_s_limit_is_named_before_anything_is_sent, fails on the previous server); the closed-meanwhile notice reads 'The upload to <name> ended: <reason>' and its galata test polls every 2 s to prove it is said once (fails without the reset)
  - log: 2026-09-24T09:55:02Z @kj 2026-09-24 the size refusal puts the limit first and the names last ('This request accepts files up to 2 B; big.txt is larger'), so the notice's 140-character cut removes only surplus names; sizes are written with one decimal as the page and the panel write them (_size_text: '5.0 GB', '1000.0 MB'); the panel shows it as the amber 'The upload was not sent: ...' like the refused drop; galata 'a file over the request limit is named and nothing is sent' and test_a_limit_is_written_as_the_page_writes_sizes fail on the previous code
...
- [x] `ACC-DRAG-156` **Drop onto an existing share** - HIGH; a file or a folder dragged from the file browser onto an existing share row is added to that share; several selected items in one drag are all added; a folder is added with its contents; the row highlights while the drag is over it and the panel says how many items were added
  ...
  - log: 2026-09-21T08:03:35Z @kj closed
- [x] `ACC-DRAG-157` **Drop zone spacing is uniform, and half the gap above the link input** - LOW; the 'Drag files here to share' zone sits 10px from the panel above and at the sides, and 7px above the link input, half the 14px it had; its text has the same padding on all four sides; before 2026-09-20 the margin was 8px top, 10px sides, 4px bottom and the padding 14px vertical, 10px horizontal
  - evidence: galata drag-drop.spec.ts drop-zone test asserts equal top and side margins, one padding value and a 7px gap to the link input; style/base.css margin: 10px 10px 3px; green 2026-09-24: mock hub 55, standalone 38, pytest 494, jest 45
  - test: open the panel and measure the zone margin and padding and the gap down to the link input: 10px on three sides, one padding value, 7px gap
  - test-tags: FUNCTIONAL
  - mechanism: 2026-09-24T19:43:01Z @kj margin: 10px 10px 3px on .jp-ShareFilesPanel-dropZone with the link row's 4px top padding gives 7px below it; one length on its padding
  - log: 2026-09-20T13:13:30Z @kj added
  - log: 2026-09-20T13:24:15Z @kj edited text
  - log: 2026-09-20T14:01:51Z @kj closed
  - log: 2026-09-24T19:04:56Z @kj amended title "Drop zone spacing is uniform" -> "Drop zone spacing is uniform, and half the gap above the link input"; text "the 'Drag files here to share' zone sits with the same gap on all four sides and its text has the same padding on all four sides; before the change the margin was 8px top, 10px sides, 4px bottom and the padding 14px vertical, 10px horizontal" -> "the 'Drag files here to share' zone sits 10px from the panel above and at the sides, and 7px above the link input, half the 14px it had; its text has the same padding on all four sides; before 2026-09-20 the margin was 8px top, 10px sides, 4px bottom and the padding 14px vertical, 10px horizontal"; reason: owner 2026-09-24: halve the gap between the drop zone and the link input, and the zone reads just 'Drag files here to share'
  - log: 2026-09-24T19:04:56Z @kj edited test "open the panel and measure the zone's computed margin and padding - all four values equal on each" -> "open the panel and measure the zone margin and padding and the gap down to the link input: 10px on three sides, one padding value, 7px gap"
  - log: 2026-09-24T19:04:56Z @kj owner 2026-09-24: drop zone margin 10px 10px 3px, so it and the link row 4px top padding leave 7px; the text is 'Drag files here to share'. Gated in drag-drop.spec.ts: three margins equal, one padding value, 7px from the zone to the input
  - log: 2026-09-24T19:43:01Z @kj mechanism updated "2026-09-20T13:13:30Z @kj one length on .jp-ShareFilesPanel-dropZone margin and one on its padding" -> "margin: 10px 10px 3px on .jp-ShareFilesPanel-dropZone with the link row's 4px top padding gives 7px below it; one length on its padding"
  - log: 2026-09-24T19:50:44Z @kj edited test-tags "MANUAL" -> "FUNCTIONAL"; evidence "galata tests/drag-drop.spec.ts 'the drop zone carries the same gap on all four sides' reads the computed style and asserts one distinct value for the four margins and one for the four paddings; style/base.css carries margin: 10px and padding: 14px" -> "galata drag-drop.spec.ts drop-zone test asserts equal top and side margins, one padding value and a 7px gap to the link input; style/base.css margin: 10px 10px 3px; green 2026-09-24: mock hub 55, standalone 38, pytest 494, jest 45"
...
- [x] `ACC-EDIT-168` **A file is moved into a folder in the same share** - HIGH; a file listed inside a share is dragged onto a folder row of that same share and moves into it: the folder row takes the drop highlight, the file leaves the level it was on, and both the folder's size and the share's item count settle without a manual refresh; a drop onto the folder the file already sits in does nothing and says nothing
  ...
  - log: 2026-09-20T17:33:07Z @kj added
  - log: 2026-09-21T08:03:35Z @kj closed
- [x] `ACC-EDIT-189` **A share is renamed from its menu** - HIGH; standalone: the share row's menu offers Rename, which asks for the new name with the current one filled in, and the row shows the new name after it
  - evidence: galata save-record.spec.ts rename test: Rename Share... opens on the current name, and the row and api/shares show the new name; green 2026-09-24: mock hub 55, standalone 38, pytest 494, jest 45
  - mechanism: 2026-09-24T19:43:01Z @kj command share-files-panel:rename-share in the standalone share menu opens InputDialog.getText and PUTs api/shares/<id>/name {name}; routes.py ShareNameHandler calls ShareStore.rename
  - test: create a share, Rename it from its menu to a new name; assert the row and api/shares show the new name
  - test-tags: FUNCTIONAL, UNIT
  - log: 2026-09-24T19:07:46Z @kj added
  - log: 2026-09-24T19:43:01Z @kj mechanism updated "2026-09-24T19:08:01Z @kj command share-files-panel:rename-share in the share menu opens a name dialog and PUTs api/shares/<id>/name {name}; routes.py ShareNameHandler calls storage.rename_share" -> "command share-files-panel:rename-share in the standalone share menu opens InputDialog.getText and PUTs api/shares/<id>/name {name}; routes.py ShareNameHandler calls ShareStore.rename"
  - log: 2026-09-24T19:50:44Z @kj closed: done
- [x] `ACC-EDIT-190` **A renamed share keeps its id and its link** - HIGH; renaming a share changes its name only: its id, its public link, its password and its files stay, and a recipient holding the link still opens it
  - evidence: test_share_rename.py keeps id, password, files, folder path and creation time; galata rename test keeps the listed link and GET of it answers 200; green 2026-09-24: mock hub 55, standalone 38, pytest 494, jest 45
  - mechanism: 2026-09-24T19:43:01Z @kj ShareStore.rename rewrites the name and the slug in the sidecar in place and moves nothing, so the id, the link, the password, the files and the folder path stay; created_at is kept in the sidecar because the rewrite moves its mtime
  - test: rename a share; assert the id and link in api/shares are unchanged and GET of the public link answers 200
  - test-tags: UNIT, FUNCTIONAL
  - log: 2026-09-24T19:07:46Z @kj added
  - log: 2026-09-24T19:43:01Z @kj mechanism updated "2026-09-24T19:08:01Z @kj storage.rename_share moves the <slug>-<id> folder and its <slug>-<id>.json sidecar to the new slug and writes name; the id, password and files travel with them, and records resolve by the -<id> suffix" -> "ShareStore.rename rewrites the name and the slug in the sidecar in place and moves nothing, so the id, the link, the password, the files and the folder path stay; created_at is kept in the sidecar because the rewrite moves its mtime"
  - log: 2026-09-24T19:50:44Z @kj closed: done
- [x] `ACC-EDIT-191` **A renamed share downloads under its new name** - MEDIUM; after a rename, Download to Current Folder and Save as Zip of the share write a folder or zip named after the new name, not the old one
  - evidence: test_share_rename.py: after a rename to Year End, save_out writes out/Year-End and out/Year-End.zip; green 2026-09-24: mock hub 55, standalone 38, pytest 494, jest 45
  - mechanism: 2026-09-24T19:43:01Z @kj save_out, the public zip (PublicShareDownloadAllHandler) and a peer's Save All take the stored slug that ShareStore.rename writes
  - test: rename a share, download it whole and as zip; assert the folder and zip carry the new name
  - test-tags: UNIT
  - log: 2026-09-24T19:07:46Z @kj added
  - log: 2026-09-24T19:21:15Z @kj mechanism updated "2026-09-24T19:08:01Z @kj the download names come from the manifest slug, which is read from the folder name, so moving the folder carries the new name" -> "save_out names the folder and the zip from the manifest name, which rename rewrites; rename drops a stored slug so an old one cannot outrank the new name"
  - log: 2026-09-24T19:43:01Z @kj mechanism updated "2026-09-24T19:21:15Z @kj save_out names the folder and the zip from the manifest name, which rename rewrites; rename drops a stored slug so an old one cannot outrank the new name" -> "save_out, the public zip (PublicShareDownloadAllHandler) and a peer's Save All take the stored slug that ShareStore.rename writes"
  - log: 2026-09-24T19:50:44Z @kj closed: done
- [x] `ACC-EDIT-192` **Edge: an empty rename is refused** - MEDIUM; a rename to an empty name or to spaces only is refused with "Missing 'name'" and the share keeps its name; Cancel changes nothing
  - evidence: test_share_rename.py: the route refuses '', three spaces and None with 400 Missing 'name' and the name unchanged; galata: spaces only shows the error toast and keeps the name; green 2026-09-24: mock hub 55, standalone 38, pytest 494, jest 45
  - mechanism: 2026-09-24T19:08:01Z @kj the handler strips the name and answers 400 Missing name when nothing is left, before storage is touched
  - test: PUT the rename with a name of three spaces; assert 400 and the name unchanged
  - test-tags: UNIT
  - log: 2026-09-24T19:07:46Z @kj added
...
- [x] `ACC-SAVE-170` **Every file in the panel saves to the current folder** - HIGH; every file row the panel lists carries one save action that writes it into the file browser's current folder, under one wording wherever it appears - an own share, a hub share, a connected peer's share and an upload under a request; the file lands with the name it has in the record, a collision is answered by the same rule the file browser uses rather than a silent overwrite, and on a hub the bytes come from the hub rather than from a local path
  ...
  - log: 2026-09-24T13:40:57Z @kj 2026-09-24 owner: no download icon on a row, the save is a menu item. A hub share's file and a hub request's upload now save from the row's context menu (Save to Current Folder), as the own and connected entries already did; the download-icon button is gone from every row. galata 'a file of a hub share saves to the current folder from its menu' and the upload fetch test assert the menu and that the row has no save button
  - log: 2026-09-24T19:14:06Z @kj owner 2026-09-24: the file save now reads 'Download to Current Folder' with the download icon (ACC-SAVE-186, ACC-SAVE-187)
- [x] `ACC-SAVE-171` **A whole share or request saves, zipped or unpacked** - HIGH; a share row and a request row each offer saving the whole record into the file browser's current folder two ways: as a single zip named after the record, or unpacked as the files themselves into a folder named after the record. Both report what they are doing while they run and name what landed, both obey the download size limit the settings carry, and a failure part-way leaves neither a half-written archive nor a half-filled folder. Unpacking into a folder rather than loose into the current directory is the assumption to check with the owner - a record holding many files would otherwise scatter them into the working directory with nothing to undo it by
  ...
  - log: 2026-09-24T13:40:57Z @kj 2026-09-24 owner: the menu entries drop 'Record' - a share, request or connected share row reads 'Save to Current Folder' and 'Save as Zip'
  - log: 2026-09-24T19:14:06Z @kj owner 2026-09-24: the unpacked whole-record save now reads 'Download to Current Folder' with the download icon (ACC-SAVE-185, ACC-SAVE-187)
- [x] `ACC-SAVE-185` **The whole-record save reads Download to Current Folder** - MEDIUM; the menu entry of a share, a request and a connected share that writes the record's files into the file browser's current folder reads 'Download to Current Folder', where it read 'Save to Current Folder'
  - evidence: galata save-record.spec.ts saves a share whole through 'Download to Current Folder'; hub-mode.spec.ts share, request and connected-share menus read it; green 2026-09-24: mock hub 55, standalone 38, pytest 494, jest 45
  - mechanism: 2026-09-24T19:08:00Z @kj the labels of share-files-panel:save-record and save-connection in src/widget.ts
  - test: open the menu of a share, a request and a connected share; assert the entry reads 'Download to Current Folder'
  - test-tags: FUNCTIONAL
  - log: 2026-09-24T19:07:46Z @kj added
  - log: 2026-09-24T19:50:44Z @kj closed: done
- [x] `ACC-SAVE-186` **A file's save reads Download to Current Folder** - MEDIUM; the menu entry of a single file row that writes the file into the file browser's current folder reads 'Download to Current Folder', on an own share, a hub share, an upload under a request and a connected share
  - evidence: galata save-record.spec.ts entry menu reads 'Download to Current Folder', 'Show in File Browser', 'Copy'; hub-mode.spec.ts hub file, upload and connected file menus and peer-origin.spec.ts pick it; green 2026-09-24: mock hub 55, standalone 38, pytest 494, jest 45
  - mechanism: 2026-09-24T19:08:00Z @kj the labels of share-files-panel:copy-entry-to-cwd, save-remote-entry, save-hub-entry and save-hub-upload in src/widget.ts
  - test: open the menu of a file row in each of the four places; assert the entry reads 'Download to Current Folder'
  - test-tags: FUNCTIONAL
  - log: 2026-09-24T19:07:46Z @kj added
  - log: 2026-09-24T19:50:44Z @kj closed: done
- [x] `ACC-SAVE-187` **Download to Current Folder carries the download icon** - LOW; every 'Download to Current Folder' menu entry shows the download icon, where the save entries showed the floppy-disk save icon
  - evidence: galata save-record.spec.ts icon test asserts the download-arrow path on the record and entry 'Download to Current Folder' entries; a zipIcon mutant fails it; green 2026-09-24: mock hub 55, standalone 38, pytest 494, jest 45
  - mechanism: 2026-09-24T19:08:00Z @kj those commands carry downloadIcon from src/icons.ts in place of saveIcon
  - test: open a record menu and a file menu; assert the entry's icon is share-files:download
  - test-tags: FUNCTIONAL
  - log: 2026-09-24T19:07:46Z @kj added
  - log: 2026-09-24T19:50:44Z @kj closed: done
