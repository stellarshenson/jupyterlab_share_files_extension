...
- [x] `ACC-CLOUD-160` **The cloud icon wears the header's own colours** - LOW; the cloud icon takes the same grey as the other header icons when off and when on; only a switch in flight takes the accent; it is not green in any state
...
- [x] `ACC-CLOUD-162` **Switching is told from on by colour and the breath** - HIGH; the header cloud icon's on and switching states draw a filled cloud: on is still, in the grey of the other header icons; switching is the accent, breathing down to nothing and back; where the system asks for reduced motion the switching cloud holds still in the accent, filled on one side of a diagonal and an outline on the other; nothing is drawn around the icon in any state
...
- [x] `ACC-HUBM-128` **Upload fetch action** - HIGH; each upload row under a request offers Save to Current Folder in its context menu, which posts the fetch route with the file browser's current directory and reports the landed path; the row carries no button for it
  ...
  - test: mock hub lists one upload; the row has no button and its menu reads Save to Current Folder with an icon; choose it and assert the mock received dest and the lab token
  - test-tags: E2E
  - log: 2026-09-03T20:51:19Z @kj added
