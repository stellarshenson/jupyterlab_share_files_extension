...
- [x] `ACC-HUBM-174` **A hub record address or a bare record id connects** - HIGH; Pasting a hub record address connects: the hub origin form /s/<policy id>/<record id>, the same address on the Cloudflare hostname, and the /hub/s/... form the hub API origin redirects to. A bare record id connects too, because on a hub the id alone identifies the record - the standalone parser refuses anything that is not an absolute URL. Anything else is refused in one sentence naming what was expected.
  - evidence: hub-mode.spec.ts 'a hub link in each form, or the id alone, connects' (hub address, /hub/s/, tunnel hostname, bare id, a workspace path refused); test_hub_handlers.py test_a_hub_link_or_a_bare_id_names_the_record
  - test: mock hub: connect with each of the three address forms and with the id alone, and see one row each time; paste a workspace path and read the refusal
  - test-tags: FUNCTIONAL
  - log: 2026-09-22T21:51:04Z @kj added
  - log: 2026-09-23T10:16:00Z @kj Verified against the mock hub (ui-tests/mock_hub.py), which serves the records/<id> routes the lab asked the hub for in HUB-API-REQUEST-read-a-record-you-do-not-own.md. The live hub does not serve them yet, so on it a connect answers the hub's 404 until it does.
  - log: 2026-09-23T10:16:01Z @kj closed
- [x] `ACC-HUBM-175` **The hub reads a record the user does not own, never the lab** - CRITICAL; A connection in hub mode is a hub call carrying the spawn token, not a fetch the lab makes against a recipient page. The lab holds no bytes of the record, opens no connection to the Cloudflare hostname and keeps no copy of its manifest; listing a connected record and saving a file out of it both go through the hub, which is what owns the bytes and the CPU in this mode.
  - evidence: hub-mode.spec.ts 'a connected record is read by the hub with the token, never from its page' (every records/<id> call carries the token, no /s/ page call); test_hub_handlers.py test_connecting_by_link_lists_the_record_and_its_entries asserts every call goes to records/
  - test: mock hub: connect to a record and expand it, then read the recorded calls - every one goes to the hub with the token, none to the record address
  - test-tags: UNIT, FUNCTIONAL
  ...
  - log: 2026-09-23T10:16:00Z @kj Verified against the mock hub (ui-tests/mock_hub.py), which serves the records/<id> routes the lab asked the hub for in HUB-API-REQUEST-read-a-record-you-do-not-own.md. The live hub does not serve them yet, so on it a connect answers the hub's 404 until it does.
  - log: 2026-09-23T10:16:01Z @kj closed
- [x] `ACC-HUBM-176` **A protected record asks for its password once and retries** - MEDIUM; Connecting to a record the owner gave a password answers with the reason rather than a failure, the panel asks for the password in one dialog whose confirm reads Connect, and the retry carries it. A wrong password says so and leaves the input open; cancelling adds no row.
  - evidence: hub-mode.spec.ts 'a protected record asks for its password once and retries' (cancel adds no row, wrong password asks again, right one connects); test_hub_handlers.py test_a_protected_record_asks_for_its_password_and_keeps_it, including a lost grant earned again
  - test: mock hub: connect to a password-protected record, cancel the dialog and see no row; connect again with the wrong password and read the refusal; connect with the right one and see the row
  - test-tags: FUNCTIONAL
  - log: 2026-09-22T21:51:17Z @kj added
  - log: 2026-09-23T10:16:00Z @kj Verified against the mock hub (ui-tests/mock_hub.py), which serves the records/<id> routes the lab asked the hub for in HUB-API-REQUEST-read-a-record-you-do-not-own.md. The live hub does not serve them yet, so on it a connect answers the hub's 404 until it does.
  - log: 2026-09-23T10:16:01Z @kj closed
- [x] `ACC-HUBM-177` **A record the user already owns is refused by name** - MEDIUM; Connecting to a record this user owns says so - the row is already in the panel - instead of adding a second row for it. Standalone does this by comparing host and base path; on a hub the record carries its owner, so the comparison is the owner the hub reports against the user the lab is spawned for.
  - evidence: hub-mode.spec.ts 'your own record is refused by name and listed once'; test_hub_handlers.py test_connecting_to_your_own_record_is_refused
  ...
  - log: 2026-09-23T10:16:00Z @kj Verified against the mock hub (ui-tests/mock_hub.py), which serves the records/<id> routes the lab asked the hub for in HUB-API-REQUEST-read-a-record-you-do-not-own.md. The live hub does not serve them yet, so on it a connect answers the hub's 404 until it does.
  - log: 2026-09-23T10:16:01Z @kj closed
- [x] `ACC-HUBM-178` **A connected share saves its files and folders, or the whole share unpacked or zipped, into the current folder** - HIGH; A connected share sits in the Connected section, expands to its files and folders, and offers what the owner's own hub share offers for getting bytes out: Save to Current Folder on each file and on each folder, and on the row Save Record to Current Folder and Save Record as Zip, under the same wording every other row uses. The hub writes the bytes into the file browser's current folder; the zip is the whole share in one archive named after it. The row carries no rename, no remove, no delete, no password control and no Cloudflare control, because none of those are the connecting user's to make.
  - evidence: hub-mode.spec.ts 'a connected share saves a file, a folder, the whole and a zip into the current folder' (files found on disk, menus carry no owner control and no Download); test_hub_handlers.py test_a_connected_share_saves_an_entry_the_whole_and_a_zip; standalone zip in peer-origin.spec.ts and test_connection_save.py
  - test: mock hub: connect to another user's share holding a file and a folder, save each from its row menu and the whole share unpacked and as a zip, and find each in the current folder; open the row menu and confirm it offers no rename, remove, delete, password or cloud action
  - test-tags: FUNCTIONAL, E2E
  ...
  - log: 2026-09-23T10:16:00Z @kj Verified against the mock hub (ui-tests/mock_hub.py), which serves the records/<id> routes the lab asked the hub for in HUB-API-REQUEST-read-a-record-you-do-not-own.md. The live hub does not serve them yet, so on it a connect answers the hub's 404 until it does.
  - log: 2026-09-23T10:16:01Z @kj closed
- [x] `ACC-HUBM-179` **Disconnecting removes the row and leaves the record alone** - MEDIUM; Disconnect on a connected record removes it from the panel and nothing else: the record stays on the hub, its owner sees no change, and connecting again restores the row. A record the owner has since closed, or whose id the hub no longer knows, shows the reason in the row rather than disappearing.
  - evidence: hub-mode.spec.ts 'disconnecting leaves the record, and a closed record says why'; test_hub_handlers.py test_disconnect_leaves_the_record_and_a_closed_record_says_why
  - test: mock hub: disconnect a connected record and see the owner still listing it; delete it on the hub and read the reason the connected row carries
  - test-tags: FUNCTIONAL
  - log: 2026-09-22T21:51:28Z @kj added
  - log: 2026-09-23T10:16:00Z @kj Verified against the mock hub (ui-tests/mock_hub.py), which serves the records/<id> routes the lab asked the hub for in HUB-API-REQUEST-read-a-record-you-do-not-own.md. The live hub does not serve them yet, so on it a connect answers the hub's 404 until it does.
  - log: 2026-09-23T10:16:01Z @kj closed
- [x] `ACC-HUBM-180` **A connected request takes dropped files and folders and tints its row while they upload** - HIGH; Files and folders dragged from the file browser onto a connected request's row, or pasted onto it, upload into that request, as a drop onto the owner's own share adds to it. The hub reads them from the workspace, so a path that leaves the workspace through a link is refused before the hub is asked, in the same words as a share. While the upload runs the row wears the progress layer every other transfer wears (ACC-PROG-172) at the fraction the hub reports, and no other row does; when it lands a notification names how many items went up, and a refusal names its reason in words.
  - evidence: hub-mode.spec.ts 'a connected request takes a dropped file and folder and tints its row while they upload' (fails with the tint removed) and 'an upload the hub refuses says why'; test_hub_handlers.py upload and link-escape tests
  - test: mock hub: connect to another user's request, drag a file and a folder onto its row and see the overlay while the hub reports the upload running and none once it lands, with the count announced and the uploaded names in the hub's recorded call; a path the hub refuses is announced with the reason; pytest: a path that leaves the workspace through a link is refused with no hub call made
  - test-tags: FUNCTIONAL, E2E
