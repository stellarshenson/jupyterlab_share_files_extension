...
- [x] `DEF-PANEL-89` **the header icon waits for a tunnel nothing asked for** - MINOR; MINOR; in hub mode with the stored default on and no record switched on, `hubTunnelLook` returns the pending look - the blinking icon, aria-pressed=mixed, 'Cloudflare sharing on - waiting for the hub' - and the hub never resolves it, because it starts its tunnel connector only for a record that wants one. It clears itself the moment the owner creates their first share, so it does not survive one walk of the primary path
  ...
  - log: 2026-09-24T17:11:46Z @kj owner 2026-09-24: the grey ring of the armed look is removed; armed draws the off silhouette, still, and the tooltip 'no tunnel up yet' and aria-pressed=true tell it from off. hub-mode.spec.ts asserts box-shadow none
...
- [x] `DEF-PANEL-103` **The breathing cloud drops under the 3:1 contrast bar the stylesheet itself cites** - MINOR; The waiting indicator breathes between opacity 1.0 and 0.5 (style/base.css, keyframes share-files-breathe). Measured against the panel background, the glyph sits at 1.92:1 in the light theme and 1.79:1 in the dark theme at the trough, and still only 2.97:1 / 2.57:1 at opacity 0.8, so no opacity below 1.0 clears the 3:1 non-text bar of WCAG 1.4.11 that the same file cites twice. For roughly half of each 2.4 s cycle across the whole 90 second wait, a low-vision owner has a 14 px indicator under the bar. The state is still carried by the glyph shape, the tooltip and aria-pressed=mixed, and the prefers-reduced-motion rule holds the glyph at full strength, which is why this is MINOR rather than a blocker. The remedy that holds the bar moves the breath off the glyph onto the ring, which also rewrites the galata assertion added in the same change set - more new surface than a MINOR earns in the round that found it.
  ...
  - log: 2026-09-24T14:06:15Z @kj owner 2026-09-24: the ring breath read as a box around the icon; the breath is back on the glyph (keyframes share-files-breathe, opacity 1 to 0.5, 2.4s ease-in-out) and the trough is under the 3:1 bar again. The reduced-motion rule still holds the glyph at full strength inside a held ring. Galata hub test polls the glyph's opacity under 0.9 and reads box-shadow none; the ring keyframes fail it
  - log: 2026-09-24T17:11:46Z @kj owner 2026-09-24: the breath runs down to opacity 0 on a sharper curve, so at the trough the glyph is not shown at all
- [x] `DEF-PANEL-104` **The tunnel arriving after the 90 second wait has no cue beyond the motion stopping** - MINOR; When the hub registers the tunnel, _drawHubCloud swaps the dashed silhouette for the filled cloud and jp-mod-active applies a static colour, both in one frame; the breathing simply stops. An owner who looked away during the roughly 90 second wait learns the tunnel is up only by looking back at a 14 px icon that has changed shape. The tooltip and the link dialog still carry the state on demand, so nothing is lost, only unannounced. The remedy is an arrival envelope on jp-mod-active, which is a new animation on the element that produced four of the nine findings in the review round that raised this, so it is recorded rather than applied there.
  ...
  - log: 2026-09-24T17:11:46Z @kj owner 2026-09-24: the arrival cue is a drop-shadow glow of the glyph itself (share-files-arrive on the svg), not a rectangle growing out of the element. Gated in hub-mode.spec.ts: at the arrival the svg runs share-files-arrive and the element's box-shadow is none
...
- [x] `DEF-HUB-19` **hub outage gives no cue on Refresh** - MAJOR; hub unreachable: Refresh spins and stops with the last lists and no message; api/shares answers 502 hub_unavailable, the panel logs it to the console only and never reaches the cloud icon's unreachable state; deferred from the 2026-09-05 review; `src/widget.ts`
  ...
  - log: 2026-09-15T18:52:44Z @kj added
  - log: 2026-09-15T21:39:59Z @kj closed: fixed
  - log: 2026-09-24T17:11:46Z @kj owner 2026-09-24: while the hub does not answer the cloud icon is hidden (hubTunnelLook returns hidden); the orange struck-through cloud is removed with its icon, and the warning on a clicked Refresh is the cue. Gated in hub-mode.spec.ts: the icon is hidden after the outage
