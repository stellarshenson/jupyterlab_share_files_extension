...
- **AC-CF20 - autostart is a user setting, OFF by default** - Settings Editor → Share Files → `tunnelAutostart` (default off - a fresh server never exposes links publicly without an explicit action); persisted server-side (`tunnel_autostart` in the CLI config) and honoured at startup: on → tunnel up, public links; off → tunnel down, private links until switched on via the cloud icon or `cloudflare start`
